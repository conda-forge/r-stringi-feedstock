...
if [[ $target_platform =~ linux.* ]] || [[ $target_platform == win-32 ]] || [[ $target_platform == win-64 ]] || [[ $target_platform == osx-64 ]]; then
  ...
  # Ensure that we are using our ICU
  $R CMD INSTALL --configure-args="--disable-icu-bundle" --build .
else
  mkdir -p $PREFIX/lib/R/library/stringi
  mv * $PREFIX/lib/R/library/stringi
